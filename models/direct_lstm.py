...
def build_direct_lstm_model(config):
    ...
    # ENCODER
    encoder_input = Input(shape=(config['data']['estimation_timestep_limit'],
                                 OUR_MODEL_IMAGE_HEIGHT, OUR_MODEL_IMAGE_WIDTH, NB_CHANNELS),
                          name="encoder_input")  # None here is sequence length

    encoder_input_flat = TimeDistributed(Flatten())(encoder_input)

    encoder_rnn = CuDNNLSTM(hidden_units, return_sequences=True, return_state=True,
                            name="encoder_rnn")
    encoder_outputs_states = encoder_rnn(encoder_input_flat)
    ...
    # DECODER
    decoder_input = Input(shape=(config['data']['prediction_timestep_limit'],
                                 NB_ANGLES + NB_SLICE),
                          name="decoder_input")
    decoder_rnn = CuDNNLSTM(hidden_units, return_sequences=True, return_state=True,
                            name="decoder_rnn")
    ...
    if config['network']['single_head']:
        ...
        estimation_rotation_xy = TimeDistributed(Dense(hidden_units, activation='tanh'),
        ...
        estimation_rotation_z = TimeDistributed(Dense(hidden_units, activation='tanh'),
        ...
        prediction_rotation_xy = TimeDistributed(Dense(hidden_units, activation='tanh'),
        ...
        prediction_rotation_z = TimeDistributed(Dense(hidden_units, activation='tanh'),
